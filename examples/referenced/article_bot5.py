...
try:
    conn = pymysql.connect(host='telegramdb.cjks7yer9qjg.ap-northeast-2.rds.amazonaws.com', port=3306, user='yongjang', passwd='yongjang', db='telegramdb', charset='utf8', use_unicode=False, init_command='SET NAMES UTF8')
...
def step110IT_1(call):
    ...
    if isFirstShown is not -1:
        ...
        articleKeyboardLink = types.InlineKeyboardButton('링크로 이동', url=WEBSERVER_DNS + "?url=" + url + "&tb=information&uid=" + str(cid))
        KeywordButton3 = types.InlineKeyboardButton('처음으로', callback_data="001")
        KeywordButton4 = types.InlineKeyboardButton('공유하기', switch_inline_query="으로부터의 검색결과 입니다.\n"+WEBSERVER_DNS + "?url=" + url + "&tb=information&uid=" + str(cid))
        articleKeyboard.row(articleKeyboardDetail, articleKeyboardLink, articleKeyboardNext)
        articleKeyboard.row(KeywordButton3,KeywordButton4)
        bot.send_message(cid, WEBSERVER_DNS + "?url=" + url + "&tb=information&uid=" + str(cid), reply_markup=articleKeyboard)
    else :
...
def step110IT_2(call):
    ...
    if isFirstShown is not -1:
        ...
        articleKeyboardLink = types.InlineKeyboardButton('링크로 이동', url=WEBSERVER_DNS + "?url=" + url + "&tb=jobs&uid=" + str(cid))
        KeywordButton3 = types.InlineKeyboardButton('처음으로', callback_data="001")
        KeywordButton4 = types.InlineKeyboardButton('공유하기', switch_inline_query="으로부터의 검색결과 입니다.\n"+WEBSERVER_DNS + "?url=" + url + "&tb=jobs&uid=" + str(cid))
        articleKeyboard.row(articleKeyboardLink, articleKeyboardNext)
        articleKeyboard.row(KeywordButton3,KeywordButton4)
        bot.send_message(cid, WEBSERVER_DNS + "?url=" + url + "&tb=jobs&uid=" + str(cid), reply_markup=articleKeyboard)
    else :
        bot.send_message(cid, "아직 준비중입니다.")
        bot.send_message(cid, "어떤 종류의 IT 글을 원하시나요?", reply_markup=step110Keyboard)
...
def step120Social_2(call):
    ...
    if isFirstShown is not -1:
        ...
        articleKeyboardLink = types.InlineKeyboardButton('링크로 이동', url=WEBSERVER_DNS + "?url=" + url + "&tb=society&uid=" + str(cid))
        KeywordButton3 = types.InlineKeyboardButton('처음으로', callback_data="001")
        KeywordButton4 = types.InlineKeyboardButton('공유하기', switch_inline_query="으로부터의 검색결과 입니다.\n"+WEBSERVER_DNS + "?url=" + url + "&tb=society&uid=" + str(cid))
        articleKeyboard.row(articleKeyboardLink, articleKeyboardNext)
        articleKeyboard.row(KeywordButton3,KeywordButton4)
        bot.send_message(cid, WEBSERVER_DNS + "?url=" + url + "&tb=society&uid=" + str(cid), reply_markup=articleKeyboard)
    else :
        bot.send_message(cid, "아직 준비중입니다.")
        bot.send_message(cid, "어떤 종류의 사회 글을 원하시나요?", reply_markup=step120Keyboard)

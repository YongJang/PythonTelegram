...
def command_start(m):
    ...
    try:
        if cid not in knownUsers:
	        ...
	        print(e)
@bot.message_handler(commands=['성북구'])
def command_jobjang(m):
    ...
    bot.send_message(cid, "상품을 선택해주세요.", reply_markup=InlineKeyboardButton)

@bot.message_handler(commands=['헤어'])
def command_jobjang(m):
    cid = m.chat.id
    bot.send_message(cid, "상품을 선택해주세요.", reply_markup=articleSelectInline)
